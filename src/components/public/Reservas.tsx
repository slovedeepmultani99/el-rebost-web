...
interface InfoData { tel: string; addr: string; whatsapp?: string }
...
const fieldStyle: React.CSSProperties = { width: "100%", fontFamily: "var(--font-karla), sans-serif", fontSize: "1rem", padding: ".78em .9em", border: "1.5px solid var(--line)", borderRadius: 12, background: "#fff", color: "var(--ink)", outline: "none", transition: ".2s" }
const labelStyle: React.CSSProperties = { display: "block", fontSize: ".74rem", fontWeight: 800, letterSpacing: ".1em", textTransform: "uppercase" as const, color: "var(--ink-soft)", marginBottom: 6 }
...
export default function Reservas({ info, horarios }: { info: InfoData; horarios: Horario[] }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (parseInt(form.guests) > 40) {
      setErrorMsg(`${tr.reservas.form.groupError} ${info.tel}`)
  ...
  const features = [
    { ic: "🍽", title: tr.reservas.comidas, sub: tr.reservas.comidasSub },
    { ic: "🌙", title: tr.reservas.cenas, sub: tr.reservas.cenasSub },
    { ic: "🔥", title: tr.reservas.grupos, sub: `${tr.reservas.gruposSub} · ${info.tel}` },
    { ic: "🅿️", title: tr.reservas.parking, sub: tr.reservas.parkingSub },
  ]

  return (
    <section id="reservas" style={{ padding: "96px 0", background: "linear-gradient(160deg, var(--wine) 0%, var(--wine-deep) 100%)", color: "var(--cream)", position: "relative", overflow: "hidden", scrollMarginTop: 78 }}>
      ...
      <div className="wrap relative">
        <div className="mob-stack" style={{ display: "grid", gridTemplateColumns: ".85fr 1.15fr", gap: 54, alignItems: "start" }}>
          {/* Info */}
          <div>
            <div className="eyebrow" style={{ color: "var(--ember-bright)" }}>{tr.reservas.eyebrow}</div>
            <h2 style={{ color: "var(--cream)", fontSize: "clamp(2rem,4vw,3rem)", marginBottom: 16 }}>{tr.reservas.title}</h2>
            <p style={{ color: "rgba(245,237,224,.82)", marginBottom: 26, fontSize: "1.05rem" }}>{tr.reservas.desc}</p>
            {features.map((f, i) => (
              <div key={i} style={{ display: "flex", gap: 13, alignItems: "flex-start", marginBottom: 18 }}>
                <span style={{ width: 40, height: 40, flexShrink: 0, borderRadius: 12, background: "rgba(245,237,224,.12)", display: "grid", placeItems: "center", color: "var(--ember-bright)", fontSize: "1.1rem" }}>{f.ic}</span>
                <div>
          ...
          <div id="res-form" className="res-card" style={{ background: "var(--bone)", color: "var(--ink)", borderRadius: 24, padding: 34, boxShadow: "0 30px 70px -20px rgba(0,0,0,.6)", overflow: "hidden" }}>
            ...
            {status === "success" ? (
              <div style={{ background: "var(--olive)", color: "#fff", borderRadius: 16, padding: 22 }}>
                <h4 style={{ fontSize: "1.3rem", color: "#fff", marginBottom: 6 }}>{tr.reservas.form.success}</h4>
                <p style={{ fontSize: ".92rem" }}>{tr.reservas.form.successSub}</p>
                <a href={waConfirmUrl()} target="_blank" rel="noopener noreferrer" style={{ display: "inline-flex", alignItems: "center", gap: 8, marginTop: 14, background: "#15803d", color: "#fff", borderRadius: 100, padding: "9px 16px", fontFamily: "var(--font-karla), sans-serif", fontWeight: 700, fontSize: ".88rem", textDecoration: "none" }}>
                  <WaIcon size={16} /> {tr.reservas.form.successWa}
                </a>
                <br />
              ...
              <form onSubmit={handleSubmit}>
                <div className="res-2col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 16 }}>
                  <div>
                    <label htmlFor="res-date" style={labelStyle}>{tr.reservas.form.date}</label>
                  ...
                  <div>
                    <label htmlFor="res-guests" style={labelStyle}>{tr.reservas.form.guests}</label>
                ...
                <fieldset style={{ border: "none", padding: 0, margin: "0 0 16px" }}>
                  <legend style={labelStyle}>{tr.reservas.form.service}</legend>
                  <div style={{ display: "flex", gap: 10, marginTop: 6 }}>
                    {(["comida", "cena"] as const).map((s) => (
                      <button key={s} type="button" onClick={() => set("service", s)} style={{ flex: 1, fontFamily: "var(--font-karla)", fontWeight: 700, padding: ".7em", borderRadius: 12, border: "1.5px solid", borderColor: form.service === s ? "var(--wine)" : "var(--line)", background: form.service === s ? "var(--wine)" : "#fff", color: form.service === s ? "var(--cream)" : "var(--ink-soft)", cursor: "pointer", transition: ".2s" }}>
                        {s === "comida" ? `🍽 ${tr.reservas.form.lunch}` : `🌙 ${tr.reservas.form.dinner}`}
                      </button>
                    ))}
                  </div>
                  {dinnerDisabled && form.date && <p style={{ fontSize: ".8rem", color: "var(--ember)", marginTop: 4 }}>{tr.reservas.form.dinnerWarning}</p>}
                </fieldset>

                <fieldset style={{ border: "none", padding: 0, margin: "0 0 16px" }}>
                  <legend style={labelStyle}>{tr.reservas.form.time}</legend>
                ...
                <div className="res-2col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 16 }}>
                  <div>
                    <label htmlFor="res-name" style={labelStyle}>{tr.reservas.form.name}</label>
                    <input id="res-name" required value={form.name} onChange={(e) => set("name", e.target.value)} placeholder="Tu nombre" style={fieldStyle} />
                  </div>
                  <div>
                    <label htmlFor="res-phone" style={labelStyle}>{tr.reservas.form.phone}</label>
                ...
                <div style={{ marginBottom: 16 }}>
                  <label htmlFor="res-notes" style={labelStyle}>{tr.reservas.form.notes}</label>
                  <textarea id="res-notes" rows={2} value={form.notes} onChange={(e) => set("notes", e.target.value)} placeholder={tr.reservas.form.notesPlaceholder} style={{ ...fieldStyle, resize: "none" }} />
                </div>
                ...
                <button type="submit" disabled={status === "loading" || !form.time || dinnerDisabled}
                  ...
                  {status === "loading" ? tr.reservas.form.sending : tr.reservas.form.submit}
                </button>
                <p style={{ textAlign: "center", marginTop: 14, fontSize: ".82rem", color: "var(--ink-soft)" }}>
                  {tr.reservas.form.whatsapp}{" "}
