...
export default function Contacto({ info, horarios }: Readonly<{ info: InfoData; horarios: readonly Horario[] }>) {
  ...
  const igHref = info.ig ? (info.ig.startsWith("http") ? info.ig : `https://instagram.com/${info.ig.replace("@", "")}`) : ""
  const igDisplay = info.ig ? igHandle(info.ig) : ""

  const items = [
    { ic: "📍", title: tr.contacto.address, content: info.addr },
    { ic: "📞", title: tr.contacto.phone, content: info.tel, href: `tel:+34${info.tel.replace(/\s/g, "")}` },
    ...(igDisplay ? [{ ic: "📸", title: tr.contacto.follow, content: igDisplay, href: igHref }] : []),
  ]

  return (
    <section id="contacto" style={{ padding: "96px 0", background: "var(--wine-deep)", color: "var(--cream)", scrollMarginTop: 78 }}>
      <div className="wrap">
        <div className="mob-stack" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 54, alignItems: "center" }}>
          <div>
            <div className="eyebrow" style={{ color: "var(--ember-bright)" }}>{tr.contacto.eyebrow}</div>
            <h2 style={{ color: "var(--cream)", fontSize: "clamp(2rem,4vw,3rem)", marginBottom: 24 }}>{tr.contacto.title}</h2>

            {items.map((c) => (
              <div key={c.title} style={{ display: "flex", gap: 14, alignItems: "flex-start", marginBottom: 18 }}>
                <span style={{ width: 42, height: 42, flexShrink: 0, borderRadius: 12, background: "rgba(245,237,224,.1)", display: "grid", placeItems: "center", color: "var(--ember-bright)", fontSize: "1.1rem" }}>{c.ic}</span>
            ...
            <div style={{ marginTop: 26, borderTop: "1px solid rgba(245,237,224,.2)", paddingTop: 18 }}>
              {horarios.map((h, i) => (
                <div key={h.dias} className="horario-row" style={{ fontSize: ".92rem", color: i === todayIdx ? "var(--ember-bright)" : "var(--cream)", fontWeight: i === todayIdx ? 700 : 400 }}>
                  <b style={{ fontWeight: 600 }}>{h.dias}{i === todayIdx ? ` · ${tr.contacto.today}` : ""}</b>
